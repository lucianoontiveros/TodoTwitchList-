...
const Controlador = (
  ...
  const usernameCroquetas = (username) =>
    croquetas.find((item) => item.username === username);
  ...
  const identificarCroquetas = (username) => {
    ...
    croquetas.find((item) => {
      if (item.username === username) {
        ...
        const perfilUsuario = perfil.find(
          (perfilItem) => perfilItem.username === username
        );
        if (perfilUsuario) {
        ...
        let mensajeCroqueta =
          item.croquetasCantidad === 1
            ? `Me regalaste una croqueta ${username} BegWan VirtualHug hoy realizaste ${item.tareasRealizadas} de los pendientes GlitchCat ,`
            : `Me regalaste una croqueta ${username} BegWan VirtualHug hoy realizaste ${item.tareasRealizadas} tareas de tu lista GlitchCat,`;
        let mensajeUsuario =
          item.croquetasCantidad === 0
            ? " No borraste niguna de las tareas registradas PogChamp "
            : ` y borraste ${item.tareasEliminadas} de las registradas PogChamp `;
        let MensajecroquetaTotal = `Me regalaste un total de ${perfilUsuario.croquetasTotal} croquetas. `;
        client.say(
          channel,
          MensajeCam + mensajeCroqueta + mensajeUsuario + MensajecroquetaTotal
        );
  ...
  switch (command) {
      ...
      perfil.find((item) => {
        if (item.username === username) {
          if (item.tareas.length >= 10) {
            ...
            client.say(
              channel,
              `Disculpe, ${username}, la petición fue rechazada por un limite maximo de 10 tareas. Complete o elimine algunas de sus tareas. Sorry, ${username}, you've reached the maximum limit of tasks (10). Complete or delete existing tasks to add new ones.`
            );
            return;
          }
          ...
          client.say(
            channel,
            `/me imGlitch @${item.username} imGlitch I registered your task: 🐱‍💻 ${tarea}  | BegWan VirtualHug Could you give me a !croqueta?`
          );
        }
      });
      break;
    case "!tarea":
      perfil.find((item) => {
        if (item.username === username) {
          ...
          client.say(
            channel,
            `/me imGlitch @${item.username} imGlitch registré tu tarea: 🐱‍💻 ${tarea} con 🔖 ${nuevoId} | BegWan VirtualHug podrías regalarme una !croqueta?`
          );
        }
      });
      break;
      ...
      perfil.find((item) => {
        if (item.username === username) {
          const listaTareas = item.tareas.forEach((i) => {
            client.say(
              channel,
              `/me imGlitch @${username} imGlitch TAREA: 📖  ${i.tarea} 🔖 !marcar ${i.id} 🔖 !eliminar ${i.id} BegWan`
            );
          });
          var anuncio =
            "/me No tiene tareas que tenga recordar. !comandos para revisar todo lo que puedo hacer BegWan VirtualHug .";
          if (item.tareas.length === 0) {
      ...
      try {
        perfil.find((item) => {
          if (item.username === username) {
            ...
            client.say(
              channel,
              `This task was eliminated: 📖 ${tareaEliminada.tarea}  with the id: 🔖 ${tareaEliminada.id} `
            );
        ...
        client.say(
          channel,
          `I have no idea what you are asking me to do. You should check the id, copy it when you throw the !list command CorgiDerp .`
        );
      }
      break;
    case "!eliminar":
      try {
        perfil.find((item) => {
          if (item.username === username) {
            ...
            client.say(
              channel,
              `Está tarea fue eliminada: 📖 ${tareaEliminada.tarea}  con el Id: 🔖 ${tareaEliminada.id} `
            );
            const tareasFiltradas = item.tareas.filter((u) => u.id !== id);
            item.tareas = tareasFiltradas;
            rendicionCroqueta(username, 0, 1);
        ...
        client.say(
          channel,
          `no tengo idea que me estas pidiendo hacer. Deberias revisar el id, copiarlo al tirar el comando !lista CorgiDerp `
        );
      }
      break;
    case "!check":
      try {
        perfil.find((item) => {
          if (item.username === username) {
            ...
            client.say(
              channel,
              `This task was marked: 📖  ${taskCheck.tarea}  with the id:  🔖 ${taskCheck.id} `
            );
            const tareasFiltradas = item.tareas.filter((u) => u.id !== id);
      ...
      try {
        perfil.find((item) => {
          if (item.username === username) {
            ...
            client.say(
              channel,
              `Esta tarea fue marcada: 📖  ${taskCheck.tarea}  con el Id:  🔖 ${taskCheck.id} `
            );
            const tareasFiltradas = item.tareas.filter((u) => u.id !== id);
            console.log(tareasFiltradas);
            item.tareas = tareasFiltradas;
    ...
    case "!clear":
      perfil.find((item) => {
        if (item.username === username) {
          client.say(
            channel,
            `Todas tus tareas fueron eliminadas ImTyping | All your tasks were deleted   ImTyping BegWan `
          );
    ...
    case "!pickup":
      perfil.find((item) => {
        if (item.username === username) {
          client.say(
            channel,
            `Todas tus tareas fueron marcadas como realizadas ImTyping | All your tasks were marked as done ImTyping BegWan `
          );
          const tareasFiltradas = item.tareas.filter((u) => u.id !== id);
    ...
    case "!eliminarusuario":
      if (username === "cuartodechenz") {
        ...
        if (
          ...
          client.say(
            channel,
            `Usuario ${removedUser.username} eliminado por cuartodechenz.`
          );
        } else {
    ...
    case "!nacionalidad":
      perfil.find((item) => {
        if (item.username === username) {
          ...
          client.say(
            channel,
            `Nacionalidad actualizada para ${username}: ${tarea}`
          );
        }
      });
      break;
          ...
          if (fechaRegex.test(tarea)) {
            ...
            switch (mes) {
              ...
              default:
                ...
                client.say(
                  channel,
                  `Corrobora que ingresaras correctamente la fecha de tu cumpleaños, el formato es DD-MM`
                );
            }
            client.say(
              channel,
              `Fecha de nacimiento actualizada para ${username}: ${tarea}. Signo zodiacal: ${item.signo}`
            );
          } else {
            client.say(
              channel,
              `Formato de fecha no válido. Por favor, usa el formato dd-mm.`
            );
          }
        }
      });
    ...
    case "!instagram":
      perfil.find((item) => {
        if (item.username === username) {
          ...
          client.say(
            channel,
            `Instagram actualizado para ${username}: ${tarea}`
          );
        }
      });
      break;
    case "!verusuario":
      ...
      if (targetUser) {
        ...
        client.say(
          channel,
          `Resumen del perfil para ${targetUsername}: Puntos - ${targetUser.puntos}, Nacionalidad - ${targetUser.nacionalidad}, Croquetas - ${targetUser.croquetasTotal}, Signo - ${targetUser.signo}, Nacimiento - ${targetUser.nacimiento}, Instagram: ${targetUser.instagram}, Ubicación ${targetUser.index}`
        );
      } else {
        client.say(
          channel,
          `No se encontró un usuario con el nombre ${targetUsername}`
        );
      }
      break;
  }
  return <div>Controlador</div>;
};
